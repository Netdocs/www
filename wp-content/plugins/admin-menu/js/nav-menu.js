...
(function($) {
	...
	api_custom = wpNavMenuCustom = {
		...
		init : function() {
			api_custom.menuList = $('#menu-to-edit1');
			api_custom.targetList = api_custom.menuList;
			...
			if ( api_custom.menuList.length )
				this.initSortables();

			if ( menus.oneThemeLocationNoMenus )
				$( '#posttype-page' ).addSelectedToMenu( api_custom.addMenuItemToBottom );
		...
		jQueryExtensions : function() {
			...
			$.fn.extend({
				menuItemDepth : function() {
					var margin = api_custom.isRTL ? this.eq(0).css('margin-right') : this.eq(0).css('margin-left');
					return api_custom.pxToDepth( margin && -1 != margin.indexOf('px') ? margin.slice(0, -2) : 0 );
				},
				...
				addSelectedToMenu : function(processMethod) {
					...
					return this.each(function() {
						...
						processMethod = processMethod || api_custom.addMenuItemToBottom;
						...
						$(checkboxes).each(function(){
							...
							if ( this.className && -1 != this.className.indexOf('add-to-top') )
								processMethod = api_custom.addMenuItemToTop;
							menuItems[listItemDBID] = t.closest('li').getItemData( 'add-menu-item', listItemDBID );
						});

						// Add the items
						api_custom.addItemToMenu(menuItems, processMethod, function(){
		...
		moveMenuItem : function( $this, dir ) {
			...
			$this.focus();
			api_custom.registerChange();
			api_custom.refreshKeyboardAccessibility();
			api_custom.refreshAdvancedAccessibility();
		},

		initAccessibility : function() {
			var menu = $( '#menu-to-edit1' );
			...
			menu.on( 'mouseenter.refreshAccessibility focus.refreshAccessibility touchstart.refreshAccessibility' , '.menu-item' , function(){
				api_custom.refreshAdvancedAccessibilityOfItem( $( this ).find( 'a.item-edit' ) );
			} );
			...
			menu.on( 'click', 'a.item-edit', function() {
				api_custom.refreshAdvancedAccessibilityOfItem( $( this ) );
			} );
			...
			menu.on( 'click', '.menus-move', function () {
				...
				if ( 'undefined' !== typeof dir ) {
					api_custom.moveMenuItem( $( this ).parents( 'li.menu-item' ).find( 'a.item-edit' ), dir );
				}
			});
		},
		...
		refreshAdvancedAccessibilityOfItem : function( itemToRefresh ) {
			...
			var thisLink, thisLinkText, primaryItems, itemPosition, title,
				...
				totalMenuItems = $('#menu-to-edit1 li').length,
		...
		refreshKeyboardAccessibility : function() {
			$( 'a.item-edit' ).off( 'focus' ).on( 'focus', function(){
				$(this).off( 'keydown' ).on( 'keydown', function(e){
					...
					case 'up':
						api_custom.moveMenuItem( $this, 'up' );
						break;
					case 'down':
						api_custom.moveMenuItem( $this, 'down' );
						break;
					case 'left':
						api_custom.moveMenuItem( $this, 'left' );
						break;
					case 'right':
						api_custom.moveMenuItem( $this, 'right' );
						break;
					}
		...
		initPreviewing : function() {
			...
			$( '#menu-to-edit1' ).on( 'change input', '.edit-menu-item-title', function(e) {
		...
		initToggles : function() {
			...
			// hide fields
			api_custom.menuList.hideAdvancedMenuItemFields();
		...
		initSortables : function() {
			var currentDepth = 0, originalDepth, minDepth, maxDepth,
				...
				menuEdge = api_custom.menuList.offset().left,
				body = $('body'), maxChildDepth,
				menuMaxDepth = initialMenuMaxDepth();

			if( 0 !== $( '#menu-to-edit1 li' ).length )
				$( '.drag-instructions' ).show();

			// Use the right edge if RTL.
			menuEdge += api_custom.isRTL ? api_custom.menuList.width() : 0;

			api_custom.menuList.sortable({
				handle: '.menu-item-handle',
				placeholder: 'sortable-placeholder',
				items: api_custom.options.sortableItems,
				start: function(e, ui) {
					...
					width += api_custom.depthToPx(maxChildDepth - originalDepth); // Account for children
				...
				change: function(e, ui) {
					...
					if( ! ui.placeholder.parent().hasClass('menu') )
						(prev.length) ? prev.after( ui.placeholder ) : api_custom.menuList.prepend( ui.placeholder );

					updateSharedVars(ui);
				},
				sort: function(e, ui) {
					var offset = ui.helper.offset(),
						edge = api_custom.isRTL ? offset.left + ui.helper.width() : offset.left,
						depth = api_custom.negateIfRTL * api_custom.pxToDepth( edge - menuEdge );

					// Check and correct if depth is not within range.
					// Also, if the dragged element is dragged upwards over
					// an item, shift the placeholder to a child position.
					if ( depth > maxDepth || offset.top < ( prevBottom - api_custom.options.targetTolerance ) ) {
						depth = maxDepth;
			...
			function updateSharedVars(ui) {
				...
				if( prev.length )
					maxDepth = ( (depth = prev.menuItemDepth() + 1) > api_custom.options.globalMaxDepth ) ? api_custom.options.globalMaxDepth : depth;
				else
					maxDepth = 0;
			}
			...
			function updateMenuMaxDepth( depthChange ) {
				...
				if ( depthChange === 0 ) {
					...
					while( ! $('.menu-item-depth-' + newDepth, api_custom.menuList).length && newDepth > 0 )
						newDepth--;
				}
		...
		attachQuickSearchListeners : function() {
			...
			$( '#nav-menu-meta' ).on( inputEvent, '.quick-search', function() {
				...
				searchTimer = setTimeout( function() {
					api_custom.updateQuickSearchResults( $this );
 				}, 500 );
			}).on( 'blur', '.quick-search', function() {
				api_custom.lastSearch = '';
			});
		},
		...
		updateQuickSearchResults : function(input) {
			 ...
			 */
			if ( q.length < minSearchLength || api_custom.lastSearch == q ) {
				return;
			}

			api_custom.lastSearch = q;
			...
			$.post( ajaxurl, params, function(menuMarkup) {
				api_custom.processQuickSearchQueryResponse(menuMarkup, params, panel);
			});
		},
		...
		addMenuItemToBottom : function( menuMarkup ) {
			$(menuMarkup).hideAdvancedMenuItemFields().appendTo( api_custom.targetList );
			api_custom.refreshKeyboardAccessibility();
			api_custom.refreshAdvancedAccessibility();
		},

		addMenuItemToTop : function( menuMarkup ) {
			$(menuMarkup).hideAdvancedMenuItemFields().prependTo( api_custom.targetList );
			api_custom.refreshKeyboardAccessibility();
			api_custom.refreshAdvancedAccessibility();
		},
		...
		attachUnsavedChangesListener : function() {
			...
			if ( 0 !== $('#menu-to-edit1').length || 0 !== $('.menu-location-menus select').length ) {
				window.onbeforeunload = function(){
					if ( api_custom.menusChanged )
			...
			$('#menu-settings-column').bind('click', function(e) {
				...
				if ( target.hasClass('nav-tab-link') ) {
					...
					if ( e.target.id && 'submit-customlinkdiv' == e.target.id )
						api_custom.addCustomLink( api_custom.addMenuItemToBottom );
					else if ( e.target.id && -1 != e.target.id.indexOf('submit-') )
						$('#' + e.target.id.replace(/submit-/, '')).addSelectedToMenu( api_custom.addMenuItemToBottom );
					return false;
				}
			});
		...
		eventOnClickMenuSave : function() {
			...
			api_custom.menuList.find('.menu-item-data-position').val( function(index) { return index + 1; } );
			window.onbeforeunload = null;
		...
		eventOnClickMenuItemDelete : function(clickedEl) {
			...
			api_custom.removeMenuItem( $('#menu-item-' + itemID) );
			api_custom.registerChange();
			return false;
		},
		...
		depthToPx : function(depth) {
			return depth * api_custom.options.menuItemDepthPerLevel;
		},

		pxToDepth : function(px) {
			return Math.floor(px / api_custom.options.menuItemDepthPerLevel);
		}

	};

	$(document).ready(function(){
	   wpNavMenuCustom.init();
    });
