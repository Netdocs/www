...
class WpsoAjax
{
	...
	public function process_page_request()
	{
		...
		else
		{
			...
			# ToDo: Use wp_remote_request() instead
			$curl = curl_init( add_query_arg( 'wpso', 'show_qs', trailingslashit( $url_no_params[0] ) ) );
			...
			if ( ! empty( $query_string ) && $query_string != 'global' && ( $status >= 200 && $status < 300 ) )
			{
				$data['html'] = '
				<p style="word-wrap:break-word;">Query-String: <b>' . esc_attr( $query_string ) . '</b></p>
				<button type="button" id="wpso-get-search-page-input" class="button button-primary button-large" data-querystring="' . esc_attr( base64_encode( $query_string ) ) . '"><i class="fa fa-arrow-right"></i>&nbsp;&nbsp;' . __( 'Get Scripts & Styles', 'wp-script-optimizer' ) . '</button>&nbsp;&nbsp;' . $clear_button . '
				<br>
			...
			{
				$data['error'] = sprintf( __( "Url cannot be loaded or you have tried to check the front page - Statuscode: %d", 'wp-script-optimizer' ), $status );
				$data['html'] = $clear_button;
			}
		}
