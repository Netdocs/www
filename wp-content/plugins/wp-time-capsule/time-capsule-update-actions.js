...
function dialog_for_changeAccount() {
	remove_other_thickbox_wptc();
	var dialog_content = '<div class="this_modal_div" style="background-color: #f1f1f1;font-family: \'open_sansregular\' !important;color: #444;padding: 0px 34px 26px 34px; left:20%; z-index:1000"><span class="dialog_close"></span><div class="pu_title">Disconnect this account?</div><div class="wcard clearfix" style="width:480px"><div class="l1">The files in your dropbox account will not be affected.<br/>But all data in the plugin will be lost.</div><a class="btn_pri" id="yes_change_acc" onclick="yes_change_acc()">Yes. Let me connect another account.</a><a class="btn_sec" id="no_change" onclick="no_change()">Cancel</a></div></div>';
	jQuery("#dialog_content_id").html(dialog_content); //since it is the first call we are generating thickbox like this
	jQuery(".wptc-thickbox").click();
	styling_thickbox_tc('change_account');
}
...
function get_deselected_nodes(){
	try{
		var deSelected = jQuery("#wptc_exc_files").fancytree('getTree').getDeSelectedNodes();
		var temp_arr = [];
		jQuery.each(deSelected ,function(i, v){
			var recheck = jQuery(".fancytree-title:contains("+basename_wptc(v.key)+")").parents('span').hasClass('fancytree-partsel');
			var extra_class = v.extraClasses;
			var partsel = v.partsel;
			if (extra_class == undefined || extra_class.indexOf("fancytree-partsel") == -1) {
				temp_arr.push(v.key);
			}
		})
		jQuery("#user_excluded_files_and_folders").val(temp_arr.join(","));
		console.log(jQuery("#user_excluded_files_and_folders").val());
	} catch(err){
		console.log(err);
	}
...
function continue_save_settings_wptc(){
	console.log("continue_save_settings_wptc");
	jQuery.post(ajaxurl, {
			action: 'continue_save_settings_wptc',
	}, function(data) {
		console.log("continue_save_settings_wptc", data);
		if (data != undefined) {
			if (data == 'callAgain') {
				continue_save_settings_wptc();
			} else {
