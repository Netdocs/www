...
class WPTC_BackupController {
	...
	public function backup_path($path, $content_flag, $always_include = null, $backup_id = null) {
		...
		if (!$this->config->get_option('gotfileslist', true)) {
			...
			$prev_counter = $this->config->get_option('gotfileslist_multicall_count');
			$prev_counter = empty($prev_counter) ? 0 : $prev_counter;
			dark_debug($prev_counter, '---------------$prev_counter-----------------');
			$counter = 0;
			foreach ($Mfile_arr as $k => $Mfile) {
				...
				if ($counter > $this->iter_count_rate_limit) {
					$counter = 0;
					if(is_wptc_timeout_cut()){
						$this->config->set_option('gotfileslist_multicall_count', $counter);
					}
				}
			}
	...
	public function execute($type = '') {
		...
		try {
			if (defined(DEFAULT_REPO) && DEFAULT_REPO && !$this->dropbox->is_authorized()) {
				WPTC_Factory::get('Debug_Log')->wptc_log_now('Your ' . DEFAULT_REPO . ' account is not authorized yet.', 'BACKUP');
			...
			if ($e->getMessage() == 'Unauthorized') {
				...
				if ($e->getMessage() == 'Cloud API not set') {
					$this->proper_backup_force_complete_exit();
				}
			}
	...
	public function  proper_backup_force_complete_exit(){
		$dbBackup = WPTC_Factory::get('databaseBackup');
		$logger = WPTC_Factory::get('logger');
	...
	public function clear_prev_repo_backup_files_record($reset_inc_exc = false) {
		...
		$logger->log('Current Repo Backup Files were Removed', 'remove_cur_repo_files_record');
	}
