...
class WPTC_Config {
	...
	public function is_main_account_authorized($email = null, $pwd = null) {
		...
		$params = array('email' => $main_account_email, 'pwd' => $main_account_pwd, 'site_url' => $site_url, 'version' => WPTC_VERSION, 'ip_address' => gethostbyname($_SERVER['HTTP_HOST']));
		...
		dark_debug($cust_info, "--------cust_info --------");
		...
		if ($this->process_service_info($cust_info)) {
			...
			if(!empty($cust_info->logged_in_but_no_plans_yet)){
				...
				if ($is_cron_service) {
					...
					if (!$this->get_option('in_progress_restore', true)) {
	...
	public function send_ip_address_to_server(){
		...
		do_cron_call_wptc('users/stats', $post_req);
	}
