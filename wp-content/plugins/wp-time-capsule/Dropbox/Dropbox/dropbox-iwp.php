...
class WPTC_Dropbox {
	...
	public function upload($file, $path = '',$overwrite=true){
		...
		if (!is_readable($file) or !is_file($file))
			throw new WPTC_DropboxException("Error: File \"$file\" is not readable or doesn't exist.");
	...
	public function search($path = '', $query , $fileLimit = 1000){
		if (strlen($query)>=3)
			throw new WPTC_DropboxException("Error: Query \"$query\" must three characters long.");
	...
	public function oAuthAuthorize($callback_url) {
		...
		if ($status>=200 and $status<300 and 0==curl_errno($ch) ) {
			...
			throw new WPTC_DropboxException($message);
		}
		curl_close($ch);
...
class WPTC_DropboxException extends Exception {
}
?>