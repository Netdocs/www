...
class WPTC_Dropbox_API {
	...
	public function putFile($file, $filename = false, $path = '', $overwrite = true) {
		if (file_exists($file)) {
			if (filesize($file) <= 157286400) {
				...
				$result = $this->fetch('POST', self::CONTENT_URL, $call, $params);
				return $result;
	...
	public function getFile($file, $outFile = false, $revision = null, $isChunkDownload = array()) {
		...
		dark_debug($response, "--------below fetch--------");
	...
	public function chunkedDownload($file, $outFile = false, $revision = null, $isChunkDownload = array(), $meta_file_download = null) {
		...
		$outFilePath = $this->getTempFolderFromOutFile(wp_normalize_path($outFile));
		$file = $this->encodePath($file);
		$call = 'files/' . $this->root . '/' . $file;
	 ...
	 * @return mixed
	 */
	public function fetch($method, $url, $call, array $params = array(), $isChunkDownload = array()) {
		// Make the API call via the consumer
		$result = $this->OAuth->fetch($method, $url, $call, $params, $isChunkDownload);
		return $result;
	}
