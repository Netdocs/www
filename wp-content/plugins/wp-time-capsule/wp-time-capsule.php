...
function wordpress_time_capsule_admin_menu() {
	$imgUrl = rtrim(plugins_url('wp-time-capsule'), '/') . '/images/wptc_logo_left_list_.png';
	$text = __('WP Time Capsule', 'wptc');
	add_menu_page($text, $text, 'activate_plugins', 'wp-time-capsule-monitor', 'wordpress_time_capsule_monitor', $imgUrl, '80.0564');
...
function check_wptc_update() {
	...
	if (version_compare('1.9.1', $installed_wptc_version) > 0) {
		...
		if (empty($staging_type) && $installed_wptc_version == '1.9.0' && is_user_purchased_pro_class('Wptc_Staging')) {
...
function is_weekly_backup_eligible(&$options){
	$last_backup_time = $options->get_option('last_backup_time');
	$timestamp_diff = time() - $last_backup_time;
	$days_diff = ((($timestamp_diff / 60) / 60) /24) + 1;
	dark_debug($days_diff, '---------$days_diff------------');
	if ($days_diff >= 7) {
		return true;
	}
	return false;
...
function wptc_init_actions(){
	if (is_admin() && current_user_can('activate_plugins')) {
		...
		add_action('wp_ajax_save_settings_wptc', 'save_settings_wptc');
		add_action('wp_ajax_continue_save_settings_wptc', 'continue_save_settings_wptc');
...
function get_dropbox_authorize_url_wptc() {
	...
	$result['authorize_url'] = $dauthorize_url . '&oauth_callback=' . network_admin_url() . 'admin.php?page=wp-time-capsule&cloud_auth_action=dropbox';
...
function save_initial_setup_data_wptc() {
	...
	$user_include_files_and_folders = (isset($data['user_include_files_and_folders'])) ? $data['user_include_files_and_folders'] : false;
	$user_excluded_files_and_folders = (isset($data['user_excluded_files_and_folders'])) ? $data['user_excluded_files_and_folders'] : false;
	$user_excluded_tables = (isset($data['user_excluded_tables'])) ? $data['user_excluded_tables'] : false;
	$user_included_tables = (isset($data['user_included_tables'])) ? $data['user_included_tables'] : false;
	...
	if (!empty($user_include_files_and_folders)) {
		$config->set_option('user_include_files_and_folders', $user_include_files_and_folders);
	}
	if (!empty($user_excluded_files_and_folders)) {
		$config->set_option('user_excluded_files_and_folders', $user_excluded_files_and_folders);
	}
	if (!empty($user_excluded_tables)) {
		$config->set_option('user_excluded_tables', $user_excluded_tables);
	}
	if (!empty($user_included_tables)) {
		$config->set_option('user_included_tables', $user_included_tables);
	}
	wptc_modify_schedule_backup();
...
function save_settings_wptc(){
	...
	$config->set_option('backup_type_setting', $data['backup_type']);
	wptc_modify_schedule_backup();
	do_action('update_auto_updater_settings_wptc');
	die("over");
}

function continue_save_settings_wptc(){
	global $settings_ajax_start_time;
	$settings_ajax_start_time = time();
	do_action('wptc_continue_exclude_files');
}
...
function is_user_purchased_pro_class($classname = false){
	if (!$classname) {
		return false;
	}

	$config = WPTC_Factory::get('config');
	$data = $config->get_option('privileges_wptc');
