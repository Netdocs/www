jQuery(document).ready(function($) {
	...
	jQuery('body').on('click', '.auto_fill_cpanel_wptc', function (){
		...
		jQuery("#dialog_content_id").html(final_html);
		jQuery(".wptc-thickbox").click();
		styling_thickbox_tc();
	});
...
function disable_staging_button_wptc(){
	...
	jQuery('#select_same_server_wptc, #select_different_server_wptc').addClass('disabled').css('cursor', 'not-allowed');
...
function enable_staging_button_wptc(){
	setTimeout(function(){
		// jQuery('.opentip-container').remove();
		jQuery('#select_same_server_wptc, #select_different_server_wptc').removeClass('disabled').css('cursor', 'pointer');
	}, 2000)
}

function progress_staging_button_wptc(){
	// jQuery('.opentip-container').remove();
	jQuery('#select_same_server_wptc, #select_different_server_wptc').addClass('disabled').css('cursor', 'progress');
}