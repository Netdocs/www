	...
	*/
	...
	function unZipFile($file, $to) {
		...
		if ( ! $GLOBALS['FileSystemObj'] || !is_object($GLOBALS['FileSystemObj']) ){
			...
			ftp_file_sys_failed_msg('Could not access file system.', true);
		}
	...
	function zipArchiveUnZip($file, $to, $neededDirs = array() ) {
		...
		if ( true !== $zopen ){
			...
			//ftp_file_sys_failed_msg('Incompatible Archive', true);
		...
		for ( $i = 0; $i < $z->numFiles; $i++ ) {
			if ( ! $info = $z->statIndex($i) ){
				...
				ftp_file_sys_failed_msg('Could not retrieve file from archive while unzipping(zipArchive).', true);
				return false;
			}
		...
		foreach ( $neededDirs as $_dir ) {
			if ( ! $GLOBALS['FileSystemObj']->mkDir($_dir, FS_CHMOD_DIR) && ! $GLOBALS['FileSystemObj']->isDir($_dir) ){ // Only check to see if the Dir exists upon creation failure. Less I/O this way.
				...
				ftp_file_sys_failed_msg('Could not create directory while unzipping(zipArchive) '.$_dir, true);
				return false;
			}
		}
		unset($neededDirs);
		...
		for ( $i = 0; $i < $z->numFiles; $i++ ) {
			...
			if ( false === $contents ){
				...
				ftp_file_sys_failed_msg('Could not extract '.$info['name'].' file from archive.', true);
				return false;
			}
			...
			if ( ! $GLOBALS['FileSystemObj']->putContents( $to . $info['name'], $contents, FS_CHMOD_FILE) ){
				...
				ftp_file_sys_failed_msg('Could not copy file '.$to . $info['name'], true);
				return false;
			}
		}
	...
	function pclZipUnZip($file, $to, $neededDirs = array()) {
		...
		if ( !is_array($archive_files) ){
			...
			ftp_file_sys_failed_msg('Incompatible Archive '.$archive->errorInfo(true), true);
			return false;
		}
	
		if ( 0 == count($archive_files) ){
			//return new WP_Error('empty_archive', __('Empty archive.'));
			ftp_file_sys_failed_msg('Empty archive', true);
			return false;
		}
		...
		foreach ( $neededDirs as $_dir ) {
			if ( ! $GLOBALS['FileSystemObj']->mkDir($_dir, FS_CHMOD_DIR) && ! $GLOBALS['FileSystemObj']->isDir($_dir) ){ // Only check to see if the dir exists upon creation failure. Less I/O this way.
				//return new WP_Error('mkdir_failed', __('Could not create directory.'), $_dir);
				ftp_file_sys_failed_msg('Could not create directory while unzipping(pclZip) '.$_dir, true);
				return false;
			}
		}
		unset($neededDirs);
		...
		foreach ( $archive_files as $file ) {
			...
			if ( ! $GLOBALS['FileSystemObj']->putContents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE) ){
				...
				ftp_file_sys_failed_msg('Could not copy file '.$to . $file['filename'], true);
				return false;
			}
		}
		return true;
			...
			if ( 'f' == $fileinfo['type'] ) {
				if ( ! $GLOBALS['FileSystemObj']->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) ) {
					...
					if ( ! $GLOBALS['FileSystemObj']->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) ){
						...
						ftp_file_sys_failed_msg('Could not copy file '.$to . $filename, true);
						return false;
					}
				}
				...
				if ( !$GLOBALS['FileSystemObj']->isDir($to . $filename) ) {
					if ( !$GLOBALS['FileSystemObj']->mkDir($to . $filename, FS_CHMOD_DIR) ){
						...
						ftp_file_sys_failed_msg('Could not create directory while copying '.$to . $filename, true);
						return false;
					}
				}
