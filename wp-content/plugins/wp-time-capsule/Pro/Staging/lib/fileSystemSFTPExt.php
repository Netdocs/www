...
class filesystemSFTPExt extends filesystemBase {
	...
	function __construct($opt='') {
		$this->method = 'sftp';
                ...
                $path = WPTC_PLUGIN_DIR.'Pro/Staging/phpseclib';
		...
		if ( empty($opt['hostname']) )
			//$this->errors->add('empty_hostname', __('SSH2 hostname is required'));
                        ftp_file_sys_failed_msg('SSH2 hostname is required', true);
		else
		...
		if ( empty ($opt['username']) ) {
			//$this->errors->add('empty_username', __('SSH2 username is required'));
                        ftp_file_sys_failed_msg('SFTP username is required', true);
		}
		...
		if ( empty ($opt['password']) ) {
			if ( !$this->keys )	//password can be blank if we are using keys
				//$this->errors->add('empty_password', __('SSH2 password is required'));
                                ftp_file_sys_failed_msg('SSH2 password is required', true);
		} else {
	...
	function connect() {
		$this->link = new Net_SFTP($this->options['hostname'], $this->options['port'], 10);

		if ( ! $this->link ) {
			//$this->errors->add('connect', sprintf(__('Failed to connect to SSH2 Server %1$s:%2$s'), $this->options['hostname'], $this->options['port']));
                        ftp_file_sys_failed_msg(sprintf('Failed to connect to SSH2 Server %1$s:%2$s', $this->options['hostname'], $this->options['port']), true);
			return false;
		}
		if ( ! $this->link->login($this->options['username'], $this->options['password']) ) {
                    ftp_file_sys_failed_msg(sprintf('Username/Password incorrect for %s', $this->options['username']), true);
                    return false;
		}
