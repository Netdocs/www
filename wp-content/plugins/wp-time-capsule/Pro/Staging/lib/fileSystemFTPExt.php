...
class filesystemFTPExt extends filesystemBase {
	...
	function __construct($opt='') {
		...
		if ( ! extension_loaded('ftp') ) {
			...
			ftp_file_sys_failed_msg('The FTP PHP extension is not available', true);
			return false;
		}
		...
		if ( empty($opt['hostname']) ){
			...
			ftp_file_sys_failed_msg('FTP hostname is required');
		}
		else
		...
		if ( empty($opt['username']) ){
			...
			ftp_file_sys_failed_msg('FTP username is required');
		}
		else
		...
		if ( empty($opt['password']) ){
			...
			ftp_file_sys_failed_msg('FTP password is required');
		}
		else
	...
	function connect() {
		if(!$this->options['hostname'] || !$this->options['username'] || !$this->options['password']){
			...
			ftp_file_sys_failed_msg('See how to add the FTP details</a>.');
			return false;
		}
		...
		if ( ! $this->link ) {
			...
			ftp_file_sys_failed_msg(sprintf('Failed to connect to the FTP server "%1$s:%2$s"', $this->options['hostname'], $this->options['port']));
			return false;
		}

		if ( ! @ftp_login($this->link,$this->options['username'], $this->options['password']) ) {
			//$this->errors->add('auth', sprintf(__('Username/Password incorrect for %s'), $this->options['username']));
			ftp_file_sys_failed_msg(sprintf('FTP username or password incorrect for "%s"', $this->options['username']));
			return false;
		}
